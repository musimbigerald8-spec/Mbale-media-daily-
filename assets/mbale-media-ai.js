/* Mbale Media AI + improved site search */
(function () {
  'use strict';
  ...
  function setupSearch() {
    ...
    function getArticles() {
      ...
      document.querySelectorAll('.story, .spotlight-card, .card, article, [data-searchable]').forEach(el => {
        ...
        if (title || text) items.push({el,title:title || text.slice(0,100),text,href:link ? link.href : ''});
      });
      return items;
    }
    ...
    function render(query) {
      ...
      const matches = getArticles().filter(item => {
        ...
        if (title === q || title.includes(q) || text.includes(q)) return true;
      ...
      matches.forEach(item => {
        ...
        } else title.style.cssText = 'display:block;font:800 19px Georgia;margin-bottom:7px;';
        ...
        box.appendChild(title); box.appendChild(snippet); results.appendChild(box);
      });
  ...
  style.textContent = `
    #mbmd-ai-launcher{position:fixed;right:18px;bottom:18px;z-index:99999;border:0;border-radius:999px;background:#1677ff;color:#fff;padding:15px 20px;font:800 14px Arial,sans-serif;box-shadow:0 8px 24px #0005;cursor:pointer}
  ...
  function mountAI(){
    ...
    const launcher=document.createElement('button'); launcher.id='mbmd-ai-launcher'; launcher.type='button'; launcher.textContent='💬 Chat With Us • Mbale Media AI';
    const panel=document.createElement('section'); panel.id='mbmd-ai-panel'; panel.setAttribute('aria-label','Mbale Media AI chat');
    ...
    form.addEventListener('submit',async function(event){event.preventDefault();const question=input.value.trim();if(!question||send.disabled)return;addMessage(question,true);input.value='';send.disabled=true;try{const main=document.querySelector('main');const pageText=main?main.innerText.slice(0,12000):document.body.innerText.slice(0,12000);const res=await fetch(ENDPOINT,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:question,pageTitle:document.title,pageUrl:location.href,pageText})});const data=await res.json();if(!res.ok)throw new Error(data.error||'AI unavailable');addMessage(data.answer||'I could not answer that right now.',false)}catch(error){addMessage('I’m having trouble connecting right now. Please try again, or contact the Mbale Media Daily newsroom.',false)}finally{send.disabled=false;input.focus()}})
  }

  function start(){setupSearch();mountAI();}
