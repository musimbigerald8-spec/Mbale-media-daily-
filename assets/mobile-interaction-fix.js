/* Mbale Media Daily — mobile menu + continuous breaking ticker v4 */
(function(){
'use strict';
function menu(){
  var b=document.getElementById('menuToggle'),n=document.getElementById('navLinks');
  if(!b||!n)return false;
  if(b.dataset.mbmdV4==='1')return true;
  b.dataset.mbmdV4='1';
  b.type='button';
...
function ticker(){
  ...
  if(!t||t.dataset.mbmdV4==='1')return false;
  t.dataset.mbmdV4='1';
  Array.prototype.slice.call(t.children).forEach(function(x){t.appendChild(x.cloneNode(true));});
  t.style.animation='mbmdTickerV4 30s linear infinite';
  return true;
}
function go(){
  var s=document.createElement('style');
  s.textContent='@keyframes mbmdTickerV4{from{transform:translateX(0)}to{transform:translateX(-50%)}}.ticker-track{will-change:transform!important}@media(max-width:850px){.nav-mobile{position:relative!important;z-index:99999!important}.nav-mobile button{position:relative!important;z-index:100000!important;pointer-events:auto!important;touch-action:manipulation!important}.nav-links.open{display:grid!important;visibility:visible!important;opacity:1!important;position:relative!important;z-index:99999!important;max-height:none!important}.ticker-track{animation-duration:24s!important}}';
